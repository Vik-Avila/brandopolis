...
| HTML y navegador | PASS por revisión de interpolaciones: texto externo escapado con escape(), estados/clases desde enums internos; notices usan textContent. CSP self, frame-ancestors none, no-referrer y nosniff; cookie HttpOnly/SameSite Strict con Max-Age derivado del vencimiento real de la sesión (F-2; nunca la extiende). Esc, Tab, foco y overflow cubiertos en navegador. |
...
Revisión independiente de Claude (2026-09-24) sobre `538e8af`: sin hallazgos HIGH; `pnpm audit --prod` re-ejecutado: No known vulnerabilities found. Ver [registro](claude-rc1-independent-review.md).
...
- Las operaciones de creación no estratégicas aún no son universalmente idempotentes. Los commits de Decision estratégica sí lo son (incluido el reintento de un commit de revisión tras respuesta perdida, F-4). La idempotencia general de creación se difiere más allá de RC1 (F-3).

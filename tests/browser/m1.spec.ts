...
test('DEMO recommendation can be rejected and then explicitly approved',async({page},info)=>{
 ...
 await navigate(page,'Experimentos y aprendizajes');await page.getByLabel('¿Qué señal esperas observar?').fill('Segunda sesión voluntaria');await page.getByRole('button',{name:'Crear experimento',exact:true}).click();await page.getByRole('button',{name:'Iniciar experimento',exact:true}).click();await expect(page.locator('#decision')).toContainText('En curso');
 ...
 await page.getByRole('button',{name:'Confirmar revisión',exact:true}).click();await page.getByRole('button',{name:'Aceptar aprendizaje',exact:true}).click();await expect(page.locator('#decision')).toContainText('Aceptado');await page.screenshot({path:`test-results/learning-${info.project.name}.png`,fullPage:true});

...
$('#blueprint').addEventListener('click',()=>{closeMenu();draft=null;$('#decision').innerHTML=`<p class="eyebrow">Blueprint · estrategia vigente</p><h2>Una visión conectada de tu marca.</h2><p>Esta vista reúne tus decisiones actuales. Cada cambio se realiza desde su decisión y conserva su historial.</p>${context.questions.map(q=>{const d=context.decisions.find(d=>d.questionId===q.id),v=context.versions.find(v=>v.id===d?.activeVersionId);return `<section class="history-item"><h3>${escape(labels[q.module])}</h3><span class="badge ${d?.reviewStatus==='NEEDS_REVIEW'?'warn':''}">${d?.reviewStatus==='NEEDS_REVIEW'?'Requiere revisión':v?'Actual · v'+v.sequence:'Por decidir'}</span><p class="current">${escape(v?.selectedOption??'Aún no hay una decisión aprobada.')}</p><p>${escape(v?.rationale??'')}</p></section>`;}).join('')}`;});
